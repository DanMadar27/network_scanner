...
def get_ip_mac_routerIP_subnetMask():
    ...
    for i in range(len(lines)):
        if "Wireless Network Connection" in lines[i] or "Local Area Connection" in lines[i]:
...
def check_if_need_to_reset(num): #check if there isn't number different then 0 or 1 in the binary number 
    lst=list(num)
