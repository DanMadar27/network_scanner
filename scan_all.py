...
def Write_Pickle(x): #write the valid ip addresses to *.txt file
    with open(r"D:\MyFolder\Json and Pickle files\valid_ip_addresses.txt","w") as handle:
        pickle.dump(x,handle)
def Load_Pickle(): #return the valid ip addresses from *.txt file
    with open(r"D:\MyFolder\Json and Pickle files\valid_ip_addresses.txt","r") as handle:
...
def pingRange(ip_list,lst,valid_ip,process_lock): #pings the list of ip addresses
    ...
    if len(divided_ip_list)>0:
        for new_ip_list in divided_ip_list:
            t=threading.Thread(target=pingRange2,args=(new_ip_list,lst,valid_ip,process_lock,thread_lock,))
            t.start()
...
def pingRange2(ip_list,lst,valid_ip,process_lock,thread_lock): #pings the list of ip addresses
    for ip in ip_list:
...
def startScan(): #start scanning the local network
    ...
    all_ip=cmdInteracting.getAll_IP()#list that every element is list of ip addresses
    valid_ip=manager.list() 
    lock=multiprocessing.Lock()
    try:
        valid_ip_file=Load_Pickle() #try loading valid_ip_addresses in case of that we scanned the local network before
        for ip in valid_ip_file:
            valid_ip.append(ip)
    except:
        pass
    for x in all_ip: # creating process to each list of ip addresses
        p=multiprocessing.Process(target=pingRange,args=(x,lst,valid_ip,lock,))
        p.start()
